...
@setu.handle()
async def setu_handle(bot: Bot, event: Event, state: T_State = State()):
...
@setu.receive()
async def setu_receive(bot: Bot, event: Event, state: T_State = State()):
...
@recall_setu.handle()
async def recall_setu_handle(bot: Bot, event: Event, state: T_State = State()):
...
@my_follow.handle()
async def my_follow_(bot: Bot, event: Event, state: T_State = State()):
...
async def no_r18_handle(bot: Bot, event: Event, state: T_State = State()):
    gid = event.group_id
    uid = event.user_id
...
async def change_rank_mode_handle(bot: Bot, event: Event, state: T_State = State()):
...
async def set_got(bot: Bot, event: Event, state: T_State = State()):
